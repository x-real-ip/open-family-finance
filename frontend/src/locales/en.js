export default {
  dateLocale: "en",
  currencyLocale: "en-GB",
  text: {
    appTitle: "Open Family Finance",
    sourceOnGitHub: "Source on GitHub",
    themeLight: "Light",
    themeDark: "Dark",
    previousMonth: "Previous month",
    nextMonth: "Next month",
    chooseMonth: "Choose a month",
    currentMonth: "Current month",
    distributionMethod: "Distribution method",
    incomeMethod: "By income",
    equalMethod: "50 / 50",
    customMethod: "Custom split",
    keepsLeft: "Keeps left",
    howComputed: "How is this calculated?",
    howComputedHeader: "How this is calculated",
    expenses: "Expenses",
    savings: "Savings",
    subtotalExpensesSavings: "Subtotal expenses + savings",
    governmentBenefit: "− Government support",
    coupleFunds: "= Shared contribution",
    buffer: "+ Buffer (margin {pct}%)",
    bufferMarginTransfer: "Buffer margin per transfer",
    salary: "Enter the net salary per person (monthly). Tap a name to change it; it applies to all months.",
    model: "Expenses plus savings, minus government support, is what you finance together. Split by income share, 50/50, or your own custom split, plus a small buffer.",
    fair: "Higher earners contribute proportionally more. After transfer, each keeps the same share of their salary.",
    fairEqual: "Each contributes the same amount, regardless of who earns more.",
    fairCustom: "You choose the ratio in which each of you contributes.",
    gov: "Benefits (child allowance, etc.) arrive on the shared account and reduce what you need to contribute.",
    exp: "Type or choose a category. Use /mo or /yr for monthly/yearly input. Percentage is the share within this section.",
    sav: "Monthly savings goals. Use /mo or /yr to switch monthly/yearly input. Counts toward what you finance together.",
    incomes: "Income",
    salarySection: "Salary",
    government: "Government support",
    addGovernment: "Add benefit",
    expensesSection: "Expenses",
    fixedCosts: "Fixed costs",
    addExpense: "Add expense",
    sortBy: "Sort by",
    sortManual: "Manual",
    sortByCategory: "Category",
    sortByName: "Name",
    dragHandle: "Drag to reorder",
    perCategory: "By category",
    incomeSplit: "Income",
    contributionSplit: "Contribution",
    marginPercentage: "Margin percentage",
    savingsSection: "Savings goals",
    addSaving: "Add savings goal",
    viewToggleAria: "Switch between the monthly view and the savings overview",
    monthView: "Monthly view",
    savingsOverviewView: "Savings overview",
    savingsGoalNamePlaceholder: "Goal name (e.g. Buffer)",
    targetAmount: "Target amount",
    targetAmountPlaceholder: "Optional",
    addSavingsGoal: "New savings goal",
    subAccountHolder: "Name",
    subAccountHolderPlaceholder: "e.g. Partner",
    subAccountBank: "Bank",
    subAccountBankPlaceholder: "e.g. Nordax Bank",
    subAccountIban: "IBAN",
    subAccountIbanPlaceholder: "e.g. DE17...",
    subAccountPlanId: "Plan/reference ID",
    subAccountPlanIdPlaceholder: "e.g. OMA_120...",
    subAccountLinkedEntry: "Linked savings entry",
    subAccountLinkedEntryInfo: "Which savings entry from the monthly view feeds this account. That entry's amount (× the share alongside it) is automatically added to this account's balance every month in the projection — nothing is entered or copied separately.",
    subAccountNoEntry: "— not linked —",
    subAccountAmountLabel: "Amount",
    subAccountAmountInfo: "The linked savings entry's amount for the month selected above — this is the same entry as in the monthly view. Changing it here applies (like everywhere in the app) to this and every following month, until you change it again; earlier months stay as they were.",
    subAccountShare: "Share %",
    subAccountPreview: "→ your share",
    subAccountInterestRate: "Interest % (annual)",
    subAccountInterestRateInfo: "Optional: this account's annual interest rate. Applied monthly to the running balance (annual rate ÷ 12), so the projection and overview show how much is interest versus your own contributions.",
    interestPortion: "of which interest",
    checkpointMonth: "Start month",
    checkpointInfo: "The projection's starting point: the month and the actual bank balance at that point (e.g. right now). Everything after this month is projected automatically (start amount + monthly contribution, plus any interest); months before it aren't shown.",
    checkpointBalance: "Start amount",
    addSubAccount: "Add account",
    projectionMonths: "Months ahead",
    combinedTotal: "Total",
    targetReached: "Target amount reached",
    noSavingsGoals: "No savings goals added yet.",
    noSubAccounts: "No accounts added for this goal yet.",
    monthColumn: "Month",
    savingsChartTitle: "Projected growth",
    statistics: "Statistics",
    statsPeriod: "Period",
    statsPeriodFrom: "From",
    statsPeriodTo: "To",
    statsPeriodReset: "All months",
    noSeries: "Navigate with the arrows to update your numbers. Charts appear once there are two or more months.",
    incomePerMonth: "Contribution per month",
    monthTotals: "Monthly totals",
    logbook: "Change log",
    loading: "Loading…",
    saved: "Saved",
    saving: "Saving…",
    restoreThisMonth: "Restore this month",
    copyMonth: "Copy month to…",
    noPrevMonths: "There are no earlier months to copy to.",
    copyAmount: "Copy amount to…",
    noMonthsToCopy: "There are no other months to copy to yet.",
    copyToPast: "To earlier months",
    copyToFuture: "To later months",
    confirmCopyMonth: "Copy the figures of {month} to {count} other month(s)? Existing data there will be overwritten.",
    copyAll: "All",
    copyNone: "None",
    selectSource: "Choose a source row",
    addOperation: "Add operation",
    save: "Save",
    delete: "Delete",
    minus: "minus",
    plus: "plus",
    times: "times",
    divide: "divide",
    descriptionPlaceholder: "Description",
    categoryPlaceholder: "Category",
    correspondent: "Correspondent",
    correspondentPlaceholder: "Correspondent",
    searchingDocument: "Searching for a document…",
    noDocumentFound: "No document found in paperless",
    openInPaperless: "open in paperless ↗",
    documentModeAuto: "Automatic",
    documentModeManual: "Manual",
    noLabelPreference: "No preference (most recent)",
    documentTypes: "Document types",
    documentTags: "Tags",
    searchDocumentsPlaceholder: "Search for a document…",
    clearPinnedDocument: "Unlink",
    duration: "Duration",
    startDate: "Start date",
    endDate: "End date",
    warningDays: "Warn (days ahead)",
    contractExpired: "Expired {days} days ago",
    contractEndingSoon: "Ends in {days} days",
    contractActiveUntil: "Runs until {date}",
    notePlaceholder: "Note for this item…",
    openLink: "Open link ↗",
    sourceRule: "Source row",
    sourceLabel: "Source",
    preview: "Preview",
    bufferMarginPct: "Buffer margin (%)",
    customPctLabel: "Custom split (%)",
    customPctAria: "{name} share (%)",
    noHistory: "Not enough data — this appears once this item has a value in multiple months.",
    perMonth: "per month",
    perYear: "per year",
    periodMonthAbbr: "/mo",
    periodYearAbbr: "/yr",
    total: "Total",
    copy: "Copy",
    amount: "Amount",
    category: "Category",
    description: "Description",
    note: "Note",
    link: "Link",
    source: "Source",
    nowTag: "now",
    logEmpty: "Not enough changes recorded yet. Value changes appear here with date, time, field, old and new value.",
    months: "{count} months",
    makesOver: "makes over · {value} p/y",
    perYearShort: "p/y",
    partnerName: "Partner {n}",
    partnerPlaceholder: "Partner {n}",
    themeToggle: "Switch to {theme} theme",
    help: "Help",
    periodToggle: "Fill monthly or yearly",
    copyAmountAria: "Copy amount to other months",
    copyMonthAria: "Copy current month to previous months",
    deleteMonth: "Delete month",
    trendNoChange: "No change vs. the previous month",
    trendHigher: "Higher than the previous month",
    trendLower: "Lower than the previous month",
    unnamed: "unnamed",
    otherCategory: "Other",
    restoreMonthFromPrevious: "Restore {month} to the figures of {source}? Manual changes in this month will be lost.",
    restoreMonthFromEmpty: "Restore {month} to the empty example? (there is no earlier month to copy from)",
    priceTrend: "Price trend",
  },
};
